...
def train(train_graph, args, model, device, optimizer):
    ...
    for i in range(args.task_num):
        ...
        prediction = metric_prediction(1, spt_output, qry_output, y_spt, args.meta_val_metric)
...
def val(val_graph, args, model, device):
    ...
    for i in range(args.val_task_num):
        ...
        spt_output = spt_output.reshape(args.num_ways, args.spt_shots, -1).mean(1)
        y_spt = y_spt[::args.spt_shots]
        prediction = metric_prediction(i, spt_output, qry_output, y_spt, args.meta_val_metric)
        acc = (prediction == y_qry).float().mean()
...
def main():
    ...
    parser.add_argument('--qry_shots', type=int, default=10)
