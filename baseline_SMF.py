...
def valtest_batch(test_graph, args, retrain=False):
    way=np.random.choice(list(range(args.num_test_classes)),size=args.num_ways,replace=False)
    spt_shot=[[] for _ in range(args.num_ways)]
    qry_shot=[[] for _ in range(args.num_ways)]
    for one in way:
        shots=np.random.choice(list(range(len(test_graph[one]))),size=args.spt_shots+args.qry_shots,replace=False)
        spt_shot[one]=shots[:args.spt_shots]
        qry_shot[one]=shots[args.spt_shots:]
...
def train_batch(train_graph, args, retrain=False):
    #i = iteration - 1
    #way = ways[i][task]
    #spt_shot = spt_shots[i][task]
    #qry_shot = qry_shots[i][task]

    way=np.random.choice(list(range(args.num_train_classes)),size=args.num_ways,replace=False)
    spt_shot=[[] for _ in range(args.num_ways)]
    qry_shot=[[] for _ in range(args.num_ways)]
    for i, one in enumerate(way):
        shots=np.random.choice(list(range(len(train_graph[one]))),size=args.spt_shots+args.qry_shots,replace=False)
        spt_shot[i]=shots[:args.spt_shots]
        qry_shot[i]=shots[args.spt_shots:]
...
def metric_prediction(j, gallery, query, train_label, metric_type):
...
def train(train_graph, args, model, device, optimizer):
    model.train()
    losses = []  # losses_q[i] is the loss on step i
    correct = []

    for i in range(args.task_num):
        x_spt, y_spt, x_qry, y_qry = train_batch(train_graph, args, retrain=True)
        ...
        prediction = metric_prediction(1, spt_output, qry_out, y_spt, args.meta_val_metric)
...
def val(val_graph, args, model, device):
    model.eval()
    accs = []

    for i in range(args.val_task_num):
        x_spt, y_spt, x_qry, y_qry = valtest_batch(val_graph, args)
        y_spt = torch.LongTensor(y_spt).to(device)
        y_qry = torch.LongTensor(y_qry).to(device)
        ...
        spt_output = spt_out.reshape(args.num_ways, args.spt_shots, -1).mean(1)
        y_spt = y_spt[::args.spt_shots]
        prediction = metric_prediction(i, spt_output, qry_out, y_spt, args.meta_val_metric)
        acc = (prediction == y_qry).float().mean()
...
def test(train_graph, test_graph, args, model, device):
    model.eval()
    accs = []
    out_mean = []
    ...
    for j in range(len(train_graph)):
        ...
        y = model(x)
        out_mean.append(y[1].cpu().data.numpy())
    train_mean = np.concatenate(out_mean, axis=0).mean(0)

    for i in range(args.test_task_num):

        x_spt, y_spt, x_qry, y_qry = valtest_batch(test_graph,
                                                   args)
...
def main():
    ...
    with open("./datasets/{}/train_test_classes.json".format(args.dataset), "r") as f:
        all_class_splits = json.load(f)
        train_classes = all_class_splits["train"]
        test_classes = all_class_splits["test"]
    args.num_train_classes=len(train_classes)
    args.num_test_classes=len(test_classes)
    ...
    for iteration in range(1, args.iterations + 1):
        ...
        train_loss, train_acc = train(train_graph, args,
                                      model,
                                      device,
                                      optimizer)
        ...
        if iteration % 20 == 0:
            ...
            val_acc, val_conf = val(test_graph, args,  model,
                                    device)
            test_acc, test_conf = test(train_graph, test_graph, args,
                                       model, device,
                                       )
            acc2.append(val_acc)
