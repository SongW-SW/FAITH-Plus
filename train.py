...
def load_data(dataset, degree_as_tag):
    ...
    if dataset in ['Letter_high','ENZYMES','Reddit','TRIANGLES']:
        ...
        #node_attribures = list(map(float, node_attribures))
        ...
        if dataset=='TRIANGLES':
            total_graph_ids=defaultdict(list)
            graph2nodes=dict()
            graph2nodes.update(train_set['graph2nodes'])
            graph2nodes.update(val_set['graph2nodes'])

            graph2edges=dict()
            graph2edges.update(train_set['graph2edges'])
            graph2edges.update(val_set['graph2edges'])

            for sets in [train_set,val_set]:
                for label, graph_ids in sets['label2graphs'].items():
                    total_graph_ids[label].extend(graph_ids)

            new_train_set=defaultdict(dict)
            new_val_set=defaultdict(dict)

            for key, graph_ids in total_graph_ids.items():
                new_graph_id=graph_ids.copy()
                np.random.shuffle(new_graph_id)
                train_graphs=new_graph_id[len(new_graph_id)//5:]
                val_graphs=new_graph_id[:len(new_graph_id)//5]

                new_train_set['label2graphs'][key]=train_graphs
                new_val_set['label2graphs'][key]=val_graphs

                for id in train_graphs:
                    new_train_set['graph2edges'][id]=graph2edges[id]
                    new_train_set['graph2nodes'][id]=graph2nodes[id]

                for id in val_graphs:
                    new_val_set['graph2edges'][id]=graph2edges[id]
                    new_val_set['graph2nodes'][id]=graph2nodes[id]

            dataset_new='TRIANGLES_new'
            #os.mkdir('./AS-MAML/data/{}'.format(dataset_new))
            pkl.dump(node_attribures,open('./AS-MAML/data/{}/{}_node_attributes.pickle'.format(dataset_new,dataset_new),'wb'))
            pkl.dump(train_set,open('./AS-MAML/data/{}/{}_train_set.pickle'.format(dataset_new, dataset_new),'wb'))
            pkl.dump(val_set,open('./AS-MAML/data/{}/{}_val_set.pickle'.format(dataset_new, dataset_new),'wb'))
            pkl.dump(test_set,open('./AS-MAML/data/{}/{}_test_set.pickle'.format(dataset_new, dataset_new),'wb'))
...
class Dataset:
    ...
    def sample_one_task(self, task_source, class_index, K_shot, query_size,test_start_idx=None):
        ...
        for index in class_index:
            ...
            if task_source==self.test_tasks and self.args.dataset_name in ['R52', 'Coil', 'Letter_high','ENZYMES','TRIANGLES','Reddit']:
                query_set.append(g_list[K_shot:K_shot + query_size])

        #during test, sample from all test samples
        append_count=0
        if task_source==self.test_tasks and test_start_idx!=None:
            if self.args.dataset_name not in ['R52', 'Coil', 'Letter_high','ENZYMES','TRIANGLES','Reddit']:
...
class Trainer:
    ...
    def train_one_step(self,mode,epoch,test_idx=None, baseline_mode=None):
        if mode=='train':
            ...
            if self.args.dataset_name not in ['R52', 'Coil']:
                ...
                first_N_class_sample = np.random.choice(list(range(self.dataset.test_classes_num)), self.N_way, replace=False)
                test_mapping={}
                for i in first_N_class_sample:
                    test_mapping[i]=len(test_mapping)
    ...
    # --calculte similarities (conduct base classification)
        ...
        sample_output_embs_query,proto_input_embs, sample_output_embs_support = self.model.construct_sample_graph(sample_input_embs, support_classes,sample_input_embs_selected)
        ...
        if mode=='train':
            ...
            #acc = metrics.accuracy_score(query_labels, query_ys_pred)

            #query_probs_npy = clf.predict_proba(query_features)
